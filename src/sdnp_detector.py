...
def run_detector(images_dir: Path, bp_dir: Path, labels_path: Path, beta: float, output_path: Path,
                 no_rotation: bool = False, batch_size: int = 150):
...
def main():
    ...
    parser.add_argument("--batch-size", type=int, default=150, help="Batch size for processing (default: 150)")
    ...
    run_detector(Path(args.images), Path(args.bp), Path(args.labels), args.beta, Path(args.output),
                 no_rotation=args.no_rotation, batch_size=args.batch_size)
